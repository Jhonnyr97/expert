<?php
    ...
    //selezione codice gruppo negozio utente loggato, codice negozio utente loggato, ruolo utente loggato
    /*Line of strato´s code deleted
    /*$context = get_context_instance(CONTEXT_COURSE, $COURSE->id);

    $infoRuolo = get_user_roles($context, $USER->id);
    $ruolo= reset($infoRuolo)->shortname;

    $gruppoNegozi=$USER->department;
    $negozio=$USER->institution;
    code of stratos I*/ 
    ...
    /*our code1*/
    $usercount = get_users(false,'',false,null,'','','','','','*','',null, $context);
    /*end our code 1*/
    /* original code2
    $usersearchcount = get_users(false, '', false, null, "", '', '', '', '', '*', $extrasql, $params);
     end original code2*/
    /*our code2*/
    $usersearchcount = get_users(false, '', false, null, "", '', '', '', '', '*', $extrasql, $params,$context);
    /*end our code2*/ 
    if ($extrasql !== '') {
...
//selezione codice gruppo negozio utente loggato, codice negozio utente loggato, ruolo utente loggato
    ...
    $negozio=$USER->institution;
    */
    		//se il codice gruppo negozi dell'utente iterato è diverso da quello dell'utente loggato e l'utente loggato non è l'admin allora non mostra quell'utente nella tabella
            /*if( ($user->department != $gruppoNegozi) && !is_siteadmin($USER)){
            	$row=[];
            } code of stratos II*/
