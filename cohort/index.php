...
if(!is_siteadmin($USER)){
    //seleziono tutti i negozi appartenenti allo stesso gruppo di negozi dell'utente loggato, per il manager
    $sql= "SELECT institution, department FROM {user} WHERE department=".$USER->department." GROUP BY institution";
    $queryCodiciNegozi = $DB->get_records_sql($sql, $params=null, $limitfrom=0, $limitnum=0);

    $codiciNegozi = array_keys($queryCodiciNegozi);

    //seleziono i gruppi dal DB dove nella descrizione c'è Global
    $sql3= "SELECT idnumber FROM {cohort} WHERE description LIKE '%Global%'";
    $queryCohort = $DB->get_records_sql($sql3, $params=null, $limitfrom=0, $limitnum=0);

    $codiciAltriGruppi = array_keys($queryCohort);

    $codiciNegozi= array_merge($codiciNegozi, $codiciAltriGruppi);
}
...
if ($showall) {
    ...
    if(is_siteadmin($USER)){
        ...
        $cohorts['totalcohorts']= count($codiciNegozi);
        $cohorts['allcohorts']= count($codiciNegozi);
        $stringaSelect='';
        foreach ($codiciNegozi as $key => $value) {
            $stringaSelect .= " idnumber='".$value."' OR ";
        }

        $stringaSelect=rtrim($stringaSelect,'OR ');

        $cohorts['cohorts']= $DB->get_records_select('cohort', $stringaSelect, $params=null, $sort='', $fields='*', $limitfrom=0, $limitnum=0);
...
foreach($cohorts['cohorts'] as $cohort) {
    ...
    //se siamo in tutti i gruppi globali per il manager mostra tutti i gruppi
    if($showall && !(is_siteadmin($USER))){
    ...
    //Gruppi globali a livello di sistema: mostra solo le righe con gli idnumber del gruppo negozi selezionato all'inizio in $codiciNegozi, per il manager
    //se è l'admin vede tutto
    if(is_siteadmin($USER)){
        $data[] = $row = new html_table_row($line);
        if (!$cohort->visible) {
            $row->attributes['class'] = 'dimmed_text';
        ...
        if ( !(in_array($cohort->idnumber, $codiciNegozi)) ) {
            if($searchquery!=''){
                $data[] = $row = new html_table_row($line);
                if (!$cohort->visible) {
                    $row->attributes['class'] = 'dimmed_text';
