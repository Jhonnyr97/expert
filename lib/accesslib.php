 ...
 * when detect the user is admin redirect to the index page (https://youniversity2.expert-italia.it/),
 * and when detect a user is teacher redirect to https://youniversity2.expert-italia.it/my/
 * July 14 of 2021  
 */
function isteacheruser($user = null) {
    global $USER, $DB;
    ...
    $role = $DB->get_record_sql("select distinct r.roleid 
                                from {role_assignments} r
                                inner join {user} u 
                                on u.id = r.userid 
                                where r.roleid in (3,4)
                                and r.userid = :userid", ['userid' => $user->id]);
