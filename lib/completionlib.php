...
class completion_info {
    ...
    public function get_tracked_users($where = '', $whereparams = array(), $groupid = 0,
             $sort = '', $limitfrom = '', $limitnum = '', context $extracontext = null, context $sitecontext = null) { //c4

        global $DB, $USER; //c4 llamamos user
        ...
        //$sql .= ' FROM (' . $enrolledsql . ') eu JOIN {user} u ON u.id = eu.id'; llamamos a la vista c2
        $sql .= ' FROM (' . $enrolledsql . ') eu JOIN {vw_user} u ON u.id = eu.id';
        if ($where) {
            $sql .= " AND $where";
            $params = array_merge($params, $whereparams);
        }
        // c3 our code 3 add code for filter register on function of department
        $role = get_user_roles($sitecontext, $USER->id); //c5 filtrar usuarios 
    ...
    public function get_progress_all($where = '', $where_params = array(), $groupid = 0,
            //$sort = '', $pagesize = '', $start = '', context $extracontext = null) { c1
            $sort = '', $pagesize = '', $start = '', context $extracontext = null, context $sitecontext = null) {
        global $CFG, $DB;

        // Get list of applicable users
        $users = $this->get_tracked_users($where, $where_params, $groupid, $sort,
                $start, $pagesize, $extracontext, $sitecontext); //c2
