 ...
 */
/*our code 5, we add a parameter $extracontext = null, WE ADD $USER ON GLOBAL*/
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
                   $firstinitial='', $lastinitial='', $page='', $recordsperpage='', $fields='*', $extraselect='', array $extraparams=null, $extracontext = null) {
    global $DB, $CFG, $USER;
    ...
    /** 
     * Administration role see all users
     * Manager role see all users that belong to its department
     * Supervisor role see all users that belong to its deparment and institution
     */
    $role = get_user_roles($extracontext, $USER->id);
    $roleid = reset($role)->roleid;
    
    if ($roleid == 1) {
        $department = $USER->department;
        $select .= " AND department = '".$department."'";
        $select .= " AND roleid != 0";
    }
    ...
    if ($roleid == 9) {
        ...
        $select .= " AND department = '".$department."'";
        $select .= " AND institution = '".$institution."'";
        $select .= " AND roleid not in (0,1)";
    }
    /**
     * End
     */
    ...
    if ($get) {
        ...
        //our code7 we change user
        return $DB->count_records_select('vw_user', $select, $params);
        //end our code 7
    }
}
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
    ...
    global $DB, $CFG, $USER; // our code 3, add variable $USER
