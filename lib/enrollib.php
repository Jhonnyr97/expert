...
function get_enrolled_sql(context $context, $withcapability = '', $groupid = 0, $onlyactive = false, $onlysuspended = false,
    ...
    /**
     * Define system context and $USER
     */
    global $USER;
    ...
    $sql = "SELECT DISTINCT {$prefix}u.id
             FROM {vw_user} {$prefix}u
           $capjoin->joins
            WHERE $capjoin->wheres";

    $role = get_user_roles($systemcontext, $USER->id);
    $roleid = reset($role)->roleid;
    $where = " AND 1=1";
    
    if ($roleid == 1) {
        $department = $USER->department;
        $where .= " AND {$prefix}u.department = '".$department."'";
        $where .= " AND {$prefix}u.roleid != 0";
    }
    ...
    if ($roleid == 9) {
        ...
        $where .= " AND {$prefix}u.department = '".$department."'";
        $where .= " AND {$prefix}u.institution = '".$institution."'";
        $where .= " AND {$prefix}u.roleid not in (0,1)";
    }
